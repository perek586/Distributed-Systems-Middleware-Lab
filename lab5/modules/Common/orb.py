...
class Request(threading.Thread):
    ...
    def run(self):
        try:
            message = json.loads(self.conn.recv(2048).decode('UTF-8'))
            print("Received request to call method {0} from remote object".format(message['method']))
            ...
            message = {
                'error' : {'name' : type(e).__name__, 'args' : e.args} 
            }
